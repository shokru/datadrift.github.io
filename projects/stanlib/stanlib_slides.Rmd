...
```{r, warning = F, message = F, echo = F, fig.align='center', fig.width=14, fig.retina = 3}
set.seed(42)
n_points <- 10^3
...
data_ml %>%
  ...
  geom_point(size = 0.5, color = "grey") + geom_smooth() + ylim(-0.5,0.5) +
  theme(text = element_text(size=24), aspect.ratio = 0.5) + xlab("Market Capitalization (score)") +
  ylab("Return")
...
- if you like sophistication, invest time in robustness checks & sensitivity analysis (HP, time windows). Do not always go for the best parameters!
